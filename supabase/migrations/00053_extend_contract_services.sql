-- Script para extender la tabla de servicios de contratos
...
ADD COLUMN IF NOT EXISTS provider_ruc VARCHAR(20),
ADD COLUMN IF NOT EXISTS provider_name VARCHAR(150);

-- Actualizar la función para recibir los nuevos parámetros
DROP FUNCTION IF EXISTS register_contract_service(UUID, VARCHAR, TEXT, DECIMAL, DATE);
...
BEGIN
    ...
    INSERT INTO public.contract_services (
        ...
        plate, driver_name, hours, provider_ruc, provider_name, created_by
    ) VALUES (
        p_contract_id, p_service_type, p_description, p_amount_pen, COALESCE(p_service_date, CURRENT_DATE),
        p_plate, p_driver_name, p_hours, p_provider_ruc, p_provider_name, auth.uid()
    );
