-- ==============================================================================
-- Migración: 20260923202000_audit_priority_three_performance.sql
-- Descripción: Creación de índices B-Tree faltantes para performance en work_orders
--              y actualización del RPC get_tower_dispatches para incluir UUIDs.
-- ==============================================================================

-- 1. Crear índices B-Tree faltantes
CREATE INDEX IF NOT EXISTS idx_work_orders_client_id ON public.work_orders(client_id);
CREATE INDEX IF NOT EXISTS idx_work_orders_transport_budget_id ON public.work_orders(transport_budget_id);

-- 2. Modificar el RPC get_tower_dispatches
DROP FUNCTION IF EXISTS public.get_tower_dispatches(date, uuid, text);
...
CREATE OR REPLACE FUNCTION public.get_tower_dispatches(
  ...
  dispatch_requests jsonb,
  dispatch_events jsonb,
  contract_codes text[],
...
BEGIN
  ...
  RETURN QUERY
  SELECT d.id,
    d.dispatch_number::text,
    d.driver_name::text,
    d.vehicle_plate::text,
    d.status::text,
    d.estimated_distance_km,
    d.scheduled_departure,
    coalesce(requests.items,'[]'::jsonb),
    coalesce(events.items,'[]'::jsonb),
    coalesce(portfolio.codes,ARRAY[]::text[]),
    coalesce(portfolio.responsibles,ARRAY[]::text[]),
    coalesce(maintenance.items,'[]'::jsonb),
    coalesce(portfolio.contract_ids, ARRAY[]::uuid[]),
    d.driver_id
  ...
  LEFT JOIN LATERAL (
    SELECT array_agg(DISTINCT c.code::text) FILTER (WHERE c.id IS NOT NULL) AS codes,
      ...
      array_agg(DISTINCT c.id) FILTER (WHERE c.id IS NOT NULL) AS contract_ids
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE mr.vehicle_plate = d.vehicle_plate
      AND (mr.status IN ('PENDIENTE', 'OBSERVADO') OR mr.severity IN ('ALTA', 'CRITICA'))
  ) maintenance ON true
...
REVOKE ALL ON FUNCTION public.get_tower_dispatches(date,uuid,text) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.get_tower_dispatches(date,uuid,text) TO authenticated;
