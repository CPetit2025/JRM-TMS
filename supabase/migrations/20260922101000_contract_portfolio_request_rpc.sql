-- Recreate the validated request RPCs with portfolio checks inside SECURITY DEFINER.
...
BEGIN
  ...
  IF v_site IS NULL OR NOT public.can_access_site(v_site) OR
     (public.is_contract_administrator() AND NOT public.has_assigned_contract(p_root_id)) THEN
    RAISE EXCEPTION 'OT no disponible para este usuario';
  END IF;
  IF p_request_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.transport_requests r WHERE r.id = p_request_id
      AND r.site_id = v_site AND r.contract_id = p_root_id
      AND public.can_access_site(r.site_id) AND
      (NOT public.is_contract_administrator() OR public.has_assigned_request(r.id))
...
BEGIN
  ...
  WHERE r.id = ANY(p_request_ids) AND public.can_access_site(r.site_id)
    AND (NOT public.is_contract_administrator() OR public.has_assigned_request(r.id))
...
BEGIN
  ...
  IF v_request.id IS NULL OR NOT public.can_access_site(v_request.site_id) OR
     (public.is_contract_administrator() AND NOT public.has_assigned_request(v_request.id,true)) THEN
