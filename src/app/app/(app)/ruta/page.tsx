                ...
                <div className={`flex-1 bg-white p-4 rounded-xl border shadow-sm ${isActive ? 'border-blue-300 ring-2 ring-blue-50' : 'border-slate-200'}`}>
                  ...
                  {isActive && (
                    <div className="mt-4 pt-4 border-t border-slate-100">
                      ...
                      <label className="block text-xs font-bold text-slate-700 mb-2">2. Registrar Llegada</label>
                      <button 
                        onClick={() => handleRegisterKM(req)}
                        disabled={processing}
                        className="w-full bg-[#002855] text-white px-4 py-3 rounded-lg text-sm font-bold shadow hover:bg-[#001f44] disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                        {processing ? <Loader2 className="w-5 h-5 animate-spin" /> : "Marcar Llegada"}
                      </button>
                    </div>
                  )}
                </div>
