...
En Windows, `scripts/android-release.ps1 init` crea una firma nueva una sola vez en `artifacts/android/signing/` y guarda la contraseña cifrada para el usuario de Windows. `scripts/android-release.ps1 build` compila y comprueba la firma con `apksigner`; `scripts/android-release.ps1 publish` además publica el APK. La publicación usa `npm run publish:apk`: sube el archivo a un bucket público exclusivo de Supabase Storage, comprueba que la URL HTTPS devuelve exactamente el mismo SHA-256 y registra la versión opcional en `app_versions`. El flujo manual `Build signed Android APK` de GitHub Actions puede reconstruir el APK con la misma firma guardada en Secrets. El keystore local y su recuperación deben respaldarse en una bóveda segura; el archivo cifrado con DPAPI solo se abre con este usuario de Windows.
